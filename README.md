...
## 1. 概要
このプロジェクトは、`ollama`と`pgvectore`と`LanChain`だけでローカル環境でRetrieval-Augmented Generation (RAG) システムを実現することを目的としています。外部APIに依存せずに、効率的な情報検索と生成を可能にし、カスタマイズされた質問応答システムを構築します。
...
- ステップ：
    ...
    1. 埋め込み生成（ベクトル化）：`ollama`で管理されているembedding modelでテキストデータをベクトル表現に変換
